...
func run(ctx context.Context, logger *slog.Logger) error {
	...
	var warmed *forecast.Forecast
	go func() {
		defer wg.Done()
		warmed = warmForecast(startupCtx, forecasts, logger)
	}()
	...
	reportColumnDrift(model, warmed, logger)
...
// returning what it fetched — or nil, if it could not.
//
// Best-effort, if forecast unavailable, just leave cache empty to be fetched when handling request.
func warmForecast(ctx context.Context, forecasts server.Fetcher, logger *slog.Logger) *forecast.Forecast {
	started := time.Now()
	warmed, err := forecasts.Fetch(ctx)
	if err != nil {
...
// reportColumnDrift warns when the forecast does not carry columns the model
// was fitted on.
//
// Secondary to internal/server's per-fetch report, which says the same thing
// and more about every forecast rather than only this one. What it adds is
// timing and a narrower question: it needs no anchor, so it answers before any
// request has been served, and a column absent from a whole response is the
// one gap that no later fetch clears — the forecast client's hardcoded
// variable list having drifted from the spec a retrain published.
//
// Silent when there is no model, or no forecast to check it against: neither
// is drift, and both have already said so in their own words.
func reportColumnDrift(model *server.Model, warmed *forecast.Forecast, logger *slog.Logger) {
	if model == nil || warmed == nil {
		return
	}
	if absent := model.AbsentColumns(warmed); len(absent) > 0 {
		logger.Warn("the forecast does not carry every column the model was fitted on",
			"absent_columns", absent, "model_version", model.VersionID)
	}
}
